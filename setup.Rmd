...
colnames(distance) <- votes$VotingPlaceID
rownames(distance) <- rownames(meshcentroids@coords)

min.vec <- apply(distance, 1, which.min)
cantymeshblocks$VotingPlaceID <- votes$VotingPlaceID[min.vec]

  
#cantymeshblocks$VotingPlaceID <- apply(distance, 1, function(x) votes$VotingPlaceID[base::which.min(distance[x])])
